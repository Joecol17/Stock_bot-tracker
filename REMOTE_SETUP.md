...
### Option A — Cloudflare Tunnel (auto-configured by setup_remote.bat)
...
- **The URL changes every restart** — bookmark it from your phone after each boot, or use Option B for a permanent URL

### Option B — Permanent URL (needs a domain)
1. Free Cloudflare account at cloudflare.com
2. Add your domain to Cloudflare (or buy one for ~£10/year)
3. Run: `cloudflared.exe tunnel login`
4. Then: `cloudflared.exe tunnel create stockbot`
5. Configure the tunnel to point to `http://localhost:5000`
6. Update `headless_launcher.py` to use named tunnel instead of `--url`
...
**Option B — Edit `docs/config.js` (permanent Named Tunnel)**  
If you set up a Cloudflare Named Tunnel (permanent URL, see Option B in *Remote dashboard access* above):

1. Edit `docs/config.js` in the repo:
   ```js
   window.STOCKBOT_API_URL = "https://stockbot.yourdomain.com";
   window.STOCKBOT_TOKEN   = "";   // fill in if using DASHBOARD_SECRET
   ```
2. Commit and push — the GitHub Pages site updates automatically
...
| `tunnel_url.txt` | Current tunnel URL (deleted on shutdown) |
| `headless_launcher.py` | Manages all processes for headless run |
| `launcher.log` | Log of the launcher process |
